...
> A real-time multiplayer coding game where each player receives one secret function to implement in pseudocode — then the entire team must deduce what larger system they just built together, with an AI judge scoring every move.
...
**CodeBlind flips this model.** Each player sees *only their function* — no one knows the overall system. Each player writes pseudocode in isolation, the team reviews all submissions together, and then collectively guesses what system they assembled. An LLM grades logic (not syntax), and another LLM verifies whether the team's guess is semantically correct.

This trains **architectural reasoning, function design, and collaborative debugging** — the skills that matter on real engineering teams.
...
| **Response format** | Structured JSON (`score`, `feedback`, `passed`) — no parsing fragility |
...
| **State management** | Zero Redux/Zustand overhead — single `GameManager` component with `useRef` socket |
...
| Metric | Detail |
|---|---|
| **Room state** | Pure in-memory JS object — zero DB reads during active gameplay |
| **DB queries per game** | 1 (puzzle fetch at `start_game`) — all subsequent operations are in-memory |
| **Deployment** | Render.com (server) + Render.com (client) — live production URLs |
...
> **Note:** The client connects to `https://codeblind-z8ay.onrender.com` by default. To point at your local server, update the `URL` constant in `src/GameManager.jsx`.
